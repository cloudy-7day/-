...
- 来源状态：外交部领事安全提醒的旧 RSS 地址已停用；在验证到可直接读取的官方 XML endpoint 前保持 inactive，不做网页抓取替代。
...
- **可同步的本地收藏**：列表与详情均可收藏，收藏页支持取消、独立查看、JSON 导入导出，并可通过 File System Access API 连接一个本地 JSON 文件自动双向合并；浏览器数据、文件句柄和删除记录分别保存在 localStorage、IndexedDB 与 v2 收藏记录中
- **简化页头**：移除洛杉矶实时时钟，只保留收藏入口、语言与明暗切换

### 数据与筛选
- 国内官方 RSS 候选池：中国新闻网、新华网、人民网和 China Daily 官方 RSS
- 外交部领事安全提醒 RSS 当前 inactive；待验证可直接读取的官方 XML endpoint 后再启用，不以 HTML scraping 替代
...
### 中期
1. 在已完成的本地文件同步收藏基础上增加评分、读后判断及真正的账号式跨设备同步。
