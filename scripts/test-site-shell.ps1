...
if ($html -notmatch 'href="#/favorites"' -or $html -notmatch 'data-favorites-count') { throw "Missing favorites navigation" }
if ($html -match 'id="clock"' -or $html -match ([regex]::Escape((-join ([char[]](0x6d1b,0x6749,0x77f6)))))) { throw "Los Angeles clock must be removed" }
...
if ($html -notmatch 'assets/fonts/source-han-serif-sc-heavy.css') { throw "Missing local display font stylesheet" }
if ($html -match 'fonts.googleapis.com|fonts.gstatic.com') { throw "Remote font CDN must not be used" }
if (-not (Test-Path (Join-Path "assets" "fonts/files/source-han-serif-sc-heavy.woff2"))) { throw "Missing local display font file" }
